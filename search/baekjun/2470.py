...
# 출력 첫째 줄에 특성값이 0에 가장 가까운 용액을 만들어내는 두 용액의 특성값을 출력한다. 출력해야 하는 두 용액은 특성값의 오름차순으로 출력한다.
...
n = int(input())
attr = sorted(list(map(int, input().split())))
...
while start <= end:
    mid = (start + end) / 2
    sum = abs(attr[start] + attr[mid])
    if sum == 0:
        print(attr[start], attr[mid], end=' ')
    else:
...
# for i in range(n):
#     for j in range(1, n):
#         sum = abs(attr[i] + attr[j])
#         if current >= sum:
#             current = sum
#             result.append((i, j, sum))
#
# print(attr[result[len(result) - 1][0]],attr[result[len(result) - 1][1]],end=' ')
